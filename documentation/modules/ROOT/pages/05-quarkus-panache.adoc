...
For the demonstration we will use https://www.postgresql.org/[PostgreSQL] as a backing database.
...
//TODO needed to be updated for PostgreSQL
ifndef::workshop[]
[tabs]
====
kubectl::
...
We will be using a pre-installed `PostgreSQL` database using `localhost` as host and `demodb` as database. 
...
[#quarkusp-access-db]
===== Accessing Database 
To be able to access Postgreqsql database locally (`localhost` if you are not using the Docker approach), you need to `port-forward` the remote port to local port.
...
[source,bash,subs="+macros,+attributes"]
----
mvn  quarkus:add-extension -Dextension="quarkus-resteasy-jsonb, quarkus-jdbc-postgresql, quarkus-hibernate-orm-panache, quarkus-smallrye-openapi"
----
...
# <.>
quarkus.datasource.url = jdbc:postgresql://postgresql:3306/demodb 
# <.>
%dev.quarkus.datasource.url = jdbc:postgresql://localhost:5432/demodb
# <.>
%fromtools.quarkus.datasource.url = jdbc:postgresql://postgresql:3306/demodb
...
<.> URL used when app is running in production (native or JAR)
<.> URL used when app is running in development mode
<.> URL used when app is running in development mode via tutorial-tools

NOTE: In tutorial-tools, we use the mariadb DNS name using the profile `fromtools`. Start Quarkus dev with this command `mvn  compile quarkus:dev -Dquarkus.profile=fromtools`.
...
2019-05-26 12:27:59,392 INFO  [io.quarkus] (main) Installed features: [agroal, cdi, hibernate-orm, jdbc-postgresql, narayana-jta, resteasy, resteasy-jsonb, smallrye-health, smallrye-openapi, swagger-ui]
----
...
public class Fruit extends PanacheEntity {
    ...
    public static List<Fruit> getAllFruitsForSeason(String season) {
        return find("LOWER(season)", season.toLowerCase()).list();
    }
